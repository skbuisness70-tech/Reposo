// db.js — sets up a local SQLite file as the database.
// SQLite is a real, production-capable database that lives in a single file
// (melamart.db). No separate database server to install or run.

const Database = require('better-sqlite3');
const path = require('path');

const db = new Database(path.join(__dirname, 'melamart.db'));
...
if (count === 0) {
  const seed = db.prepare(`
    ...
    VALUES (@title, @category, @price, @mrp, @emoji, @rating, @reviews, @badge, @description)
  `);
  ...
  insertMany(products);
  console.log(`Seeded ${products.length} products into melamart.db`);
}
